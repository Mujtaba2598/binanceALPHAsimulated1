...
const SIMULATED_BALANCE = 10000;  // Start with $10,000 simulated balance
...
// ========== REAL BINANCE MARKET DATA (No Trading) ==========
...
async function getRealBinanceOrderBook(symbol) {
    try {
        const response = await axios.get(`${BINANCE_API}/api/v3/depth?symbol=${symbol}&limit=10`);
        return {
...
// ========== API KEY MANAGEMENT (Simulated - No Real Keys Needed) ==========
...
// ========== SIMULATED TRADING ENGINE (Uses Real Prices) ==========
...
app.post('/api/start-trading', authenticate, async (req, res) => {
    try {
        ...
        const profitTargetPercent = profitPercent || 1; // Default 1% profit target
...
async function startSimulatedTrading(sessionId) {
    ...
    for (let i = 0; i < session.activeTrades.length; i++) {
        ...
        if (trade.status === 'BUY_ORDER_PLACED') {
            // Check if buy order would fill based on real market price
            const currentPrice = await getRealBinancePrice(trade.symbol);
            
            // Simulate buy order fill when market price drops to or below buy price
            if (currentPrice <= trade.buyPrice) {
                trade.status = 'BUY_FILLED';
                trade.fillPrice = trade.buyPrice;
                trade.filledQuantity = trade.quantity;
                trade.fillTime = Date.now();
                console.log(`📊 SIMULATED BUY FILLED: ${trade.filledQuantity} ${trade.symbol} at $${trade.fillPrice} (Real price: $${currentPrice})`);
                
                // Calculate sell price based on profit target
                ...
                console.log(`📈 SELL ORDER PLACED: Target $${sellPrice} (${session.profitTargetPercent}% profit)`);
            }
        } else if (trade.status === 'SELL_ORDER_PLACED') {
            // Check if sell order would fill based on real market price
            const currentPrice = await getRealBinancePrice(trade.symbol);
            
            // Simulate sell order fill when market price rises to or above sell price
            if (currentPrice >= trade.sellPrice) {
                ...
                console.log(`✅ SIMULATED SELL FILLED! Profit: $${profit.toFixed(2)}. New balance: $${session.currentBalance.toFixed(2)} (Real price: $${currentPrice})`);
    ...
    // Calculate investment per trade (minimum $10)
    let investmentPerTrade = Math.max(10, Math.min(session.currentBalance * 0.15, remainingNeeded / tradesToPlace));
    ...
    // Place new simulated trades using REAL market data
    ...
    for (let i = 0; i < tradesToPlace; i++) {
        ...
        // Use real best bid for buy price
        const bestBid = orderBook.bids[0] || currentPrice;
        const buyPrice = bestBid * 0.999; // 0.1% below best bid
        ...
        const simulatedOrderId = Date.now() + i;
        
        session.activeTrades.push({
            symbol: symbol,
            quantity: roundedQty,
            buyPrice: buyPrice,
            buyOrderId: simulatedOrderId,
            status: 'BUY_ORDER_PLACED',
            createdAt: Date.now()
        });
        newTradesPlaced++;
        
        console.log(`📊 SIMULATED BUY ORDER #${newTradesPlaced}: $${investmentPerTrade.toFixed(2)} → ${roundedQty} ${symbol} at $${buyPrice} (Target: ${session.profitTargetPercent}% profit) | Real price: $${currentPrice} | Active: ${session.activeTrades.length}`);
    }
    
    if (newTradesPlaced > 0) {
        console.log(`✅ Placed ${newTradesPlaced} simulated orders. Total active: ${session.activeTrades.length}/${MAX_CONCURRENT_TRADES}`);
